...
namespace Jgp.Issues.Tests
{
    public abstract class IssuesTestBase : AbpIntegratedTestBase<IssuesTestModule>
    {
        ...
        protected void UsingDbContext(Action<IssuesDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<IssuesDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<IssuesDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<IssuesDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<IssuesDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IssuesDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<IssuesDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IssuesDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<IssuesDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<IssuesDbContext, Task<T>> func)
        {
            T result;
